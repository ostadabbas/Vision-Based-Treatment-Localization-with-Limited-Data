...
def extract_keypoints(heatmap, all_keypoints, total_keypoint_num):
    ...
    PARAMETERS:
        heatmap: Heatmap (np.ndarray)
        all_keypoints: List of keypoints (list)
        total_keypoint_num: Total number of keypoints (int)
        
    OUTPUT:
        keypoint_num: Number of keypoints (int)
    '''
...
def connections_nms(a_idx, b_idx, affinity_scores):
    ...
    PARAMETERS:
        a_idx: Index of the first keypoint (np.ndarray)
        b_idx: Index of the second keypoint (np.ndarray)
        affinity_scores: Affinity scores (np.ndarray)
...
def group_keypoints(all_keypoints_by_type, pafs, pose_entry_size=20, min_paf_score=0.05):
    ...
    PARAMETERS:
        all_keypoints_by_type: List of keypoints (list)
        pafs: Part affinity fields (np.ndarray)
        pose_entry_size: Size of the pose entry (int)
        min_paf_score: Minimum PAF score (float)
        
    OUTPUT:
        pose_entries: List of poses (list)
        all_keypoints: List of keypoints (list)
    '''
